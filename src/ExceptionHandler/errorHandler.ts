...
export const errorHandler = (error: Error, req: Request, res: Response, next: NextFunction)=>{
    if(error instanceof UserAlreadyExistError){
        return res.status(400).json({message:"User Already Exist", success: false, error: error.message});
    }

    if(error instanceof ResourceNotFoundError){
        return res.status(404).json({message:"User Not found!", success: false, error: error.message});
    }

    if(error instanceof InvalidCredentialsError){
        return res.status(400).json({message:"Invalid credentials!", success: false, error: error.message});
    }
